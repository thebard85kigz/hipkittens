...
__global__ void __attribute__((amdgpu_num_vgpr(24))) attend_ker(const attn_globals<D> g) {
    ...
    constexpr int scale_max_reg = 29; // scale max register
    constexpr int other_norm_reg = 30; // other norm register
    ...
    for (int j = 3; j < num_tiles - 1; j += 2) {
        ...
        zero<other_norm_reg>();
        col_sum<other_norm_reg>(ATTN_0_reg);
        ...
        sub_col<max_reg>(ATTN_1_reg, ATTN_1_reg);
