...
__global__ void __attribute__((amdgpu_num_vgpr(22))) attend_ker(const attn_globals<D> g) {
    ...
    using Q_float_ranges = ducks::rt::split_many_t<ducks::rt::type_list<ducks::rt::range<160, 223>>, 8>; // 64 registers - v[160:223]
    ...
    rt<float, Q_BLOCK_SIZE, D, row_l, rt_32x16_s, Q_float_ranges> Q_float_reg; // 32x128
    ...
    rt<float, D, Q_BLOCK_SIZE, col_l, rt_32x32_s, O_ranges_transposed> O_reg_transposed; // 128x32
    ...
    // Load in the first slice of K into shared memory
    G::load<1, false>(k_smem[0], g.Kg, {batch_idx, 0, head_idx_kv, 0});

    // Load in my Q slice into registers
    ...
    for (int j = 3; j < num_tiles - 1; j += 2) {
        ...
        // Cluster 2:
        // A0V0
        mma_AtB(O_reg_transposed, V_reg, ATTN_0_bf16_in_reg, O_reg_transposed);
        ...
        // Rescale O
        mul_col<scale_reg>(O_reg_transposed, O_reg_transposed);
        __builtin_amdgcn_s_barrier();
        ...
        // Cluster 6:
        // A1V1
        mma_AtB(O_reg_transposed, V_reg, ATTN_1_bf16_in_reg, O_reg_transposed);
    ...
    mma_AtB(O_reg_transposed, V_reg, ATTN_1_bf16_in_reg, O_reg_transposed);
    float norm;
    macros::v_mov_b32_e32<norm_reg>(norm);
    div_col<norm_reg>(O_reg_transposed, O_reg_transposed);
    __builtin_amdgcn_s_barrier();
    ...
    copy(O_reg_bf16_transposed, O_reg_transposed);
    store<1>(g.Og, O_reg, {batch_idx, 0, head_idx, 0}, {0, tile_idx, 0, 0});

    // multiply by ln(2)
    mul<max_reg, max_reg>(0.69314718056f);
    norm = __logf(norm);
    macros::v_add_f32_e32<max_reg>(norm);
    store<typename rt<float, KV_BLOCK_SIZE, Q_BLOCK_SIZE, col_l, rt_32x32_s, ATTN_0_ranges>::row_vec>(g.L_vec, norm, {batch_idx, head_idx, 0, tile_idx});
}
